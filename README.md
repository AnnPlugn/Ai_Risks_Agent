...
Код
.py, .js, .java
Функции, классы, импорты, промпты в коде


Документы
.docx, .xlsx, .pdf
Описания, техспеки, инструкции


Конфигурации
.json, .yaml, .env
Настройки, промпты, ограничения


Тексты
.txt, .md
Промпты, guardrails, описания
...
Расширьте RiskType enum в risk_models.py
Создайте новый агент-оценщик в evaluator_agents.py
Обновите workflow в graph_builder.py
Добавьте промпты в config/prompts/

Интеграция новых LLM

Расширьте LLMClient в llm_client.py
...
Версия системы: 1.0.0Дата релиза: Декабрь 2024Статус: Production Ready```
