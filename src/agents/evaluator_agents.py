...
from ..prompts.call_evaluator_prompts import (call_ethical_prompt, call_social_risk_prompt, call_regulatory_risk_prompt,
                                              call_autonomy_risk_prompt, call_security_risk_prompt,
                                              call_stability_risk_prompt)
...
class EthicalRiskEvaluator(EvaluationAgent):
    ...
    def get_system_prompt(self) -> str:
        try:
            ...
            bound_logger.info(
                f"📋 EthicalRiskEvaluator использует ОБНОВЛЕННЫЙ промпт (версия с требованиями 800+ символов)")
        except:
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка этических рисков с полными данными профайлера"""
        start_time = datetime.now()

        try:
            with LogContext("evaluate_ethical_risk", assessment_id, self.name):
                # НОВОЕ: Получаем ВСЕ данные от профайлера
                agent_profile = input_data.get("agent_profile", {})
                llm_analysis_results = input_data.get("llm_analysis_results", {})
                architecture_graph = input_data.get("architecture_graph", "")

                # НОВОЕ: Используем наследуемый метод для формирования данных
                agent_data = self._format_enhanced_agent_data(
                    agent_profile, llm_analysis_results, architecture_graph
                )

                print(
                    f"🔍 DEBUG EthicalRiskEvaluator: Используем расширенные данные (размер: {len(agent_data)} символов)")
                ...
                # Проверяем threat_assessments
                if "threat_assessments" not in evaluation_result:
                    print("❌ КРИТИЧЕСКАЯ ОШИБКА: threat_assessments отсутствует в ответе!")
                    evaluation_result["threat_assessments"] = {
                        # ... fallback данные как раньше
                    }
...
class StabilityRiskEvaluator(EvaluationAgent):
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка рисков стабильности с полными данными"""
        start_time = datetime.now()
        ...
        try:
            with LogContext("evaluate_stability_risk", assessment_id, self.name):
                ...
                # НОВОЕ: Используем наследуемый метод
                agent_data = self._format_enhanced_agent_data(
                    agent_profile, llm_analysis_results, architecture_graph
                )
...
class SecurityRiskEvaluator(EvaluationAgent):
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка рисков безопасности с полными данными"""
        start_time = datetime.now()
...
class AutonomyRiskEvaluator(EvaluationAgent):
    ...
    def get_system_prompt(self) -> str:
        try:
            ...
            bound_logger = self.logger.bind_context(temp_assessment_id, self.name)
            bound_logger.info(
                f"📋 AutonomyRiskEvaluator использует ОБНОВЛЕННЫЙ промпт (версия с требованиями 1000+ символов)")
        except:
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка рисков автономности с полными данными"""
        start_time = datetime.now()
...
class RegulatoryRiskEvaluator(EvaluationAgent):
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка регуляторных рисков с полными данными"""
        start_time = datetime.now()
...
class SocialRiskEvaluator(EvaluationAgent):
    ...
    async def process(
        ...
        """УЛУЧШЕННАЯ оценка социальных рисков с полными данными"""
        start_time = datetime.now()
